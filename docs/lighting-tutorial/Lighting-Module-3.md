...
- A basic understanding of the Horizon Worlds scripting environment (how to create and attach scripts).

---

## ⚙️ Project: The Interactive Lamp

Our goal is to create a grabbable lamp with a `DynamicLightGizmo`. A player will be able to pick it up and use their controller buttons to:
- Turn it on and off.
- Cycle through settings like Hue, Saturation, Intensity, and Falloff.
- Adjust the values of the selected setting.
- Activate an "Auto Mode" for a cool, color-cycling effect.
- See the current settings on a text display.

*[Image: A player in VR holding the finished interactive lamp, with the TextGizmo displaying its current settings.]*
...
### 1. Scene Setup
First, we need to assemble the parts of our lamp in the editor.

1.  **Create a Parent Object**: Create a basic shape (like a Cylinder) that will serve as the lamp's base. In its Properties, make it **Grabbable** and **Physical**. Let's name it `InteractiveLamp`.
2.  **Add the Light**: Drag a **Dynamic Light Gizmo** into the scene and make it a child of the `InteractiveLamp` object.
3.  **Add the Display**: Drag a **Text Gizmo** into the scene and also make it a child of the `InteractiveLamp`. This will be our UI. Position it so it's easily readable when you hold the lamp.

Your scene hierarchy should look like this:
-   `InteractiveLamp` (Grabbable, Physical)
    -   `DynamicLightGizmo`
    -   `TextGizmo`

*[Image: The scene hierarchy panel showing the nested lamp, light, and text objects.]*

### 2. Create and Attach the Script
Now let's add the code.

1.  In the **Assets** panel, create a new script named `lamp`.
2.  Open the script and **delete all the default code**.
3.  **Copy and paste** the entire script provided below into your `lamp` script file.
4.  Select the parent `InteractiveLamp` object in your scene.
5.  In the Properties panel, click **Add Component** and add the `lamp` script.

<details>
<summary>Click to view the full lamp.ts script</summary>
